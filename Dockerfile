...
# 현재경로의 모든 파일들을 컨테이너의 /srv/taling 폴더에 복사
COPY            . /srv/taling

# cd /srv/taling와 같은 효과
WORKDIR         /srv/taling
...
#   home에 가상환경 적용후 실행
# /root/.pyenv/versions/taling/bin/uwsgi --http :8000 --chdir /srv/taling/django_app --home /root/.pyenv/versions/taling -w config.wsgi.debug
#
# 아래는 위 내용을 줄바꿈 처리
#
# /root/.pyenv/versions/taling/bin/uwsgi \
# --http :8000 \
# --chdir /srv/taling/django_app \
# --home /root/.pyenv/versions/taling -w config.wsgi.debug
#
#
